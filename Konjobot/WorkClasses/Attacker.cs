   ...
   public class Attacker
    {
       ...
       {         
           ...
           while (Core.client.Player.RedSquare > 0)
           {
               ...
               t.DoStance(c, GetAroundTargets(),t);
       ...
       private Creature GetNextTarget12()
       {
       ...
       {            
              List<Creature> GoodTargets = new List<Creature>();
              foreach (Objects.Bot.Target t in Core.Global.TargetList)
              {
                  foreach (Creature c in Core.client.Battlelist.GetScreenCreatures())
                  {
                      if(t.Name.ToLower() ==c.Name.ToLower())
                      {
                          if(c.IsReachable())
                          {
                              c.Cost = (int)c.Location.DistanceTo(Core.client.PlayerLocation) + t.Prio * -1;
                              GoodTargets.Add(c);
                          }
                      }
                  }
              }
           TargetsAround = GoodTargets.Count();
           if(GoodTargets.Count() > 0)
           {
               // return the lowest cost creature
                 return  GoodTargets.OrderBy(k => k.Cost).ToList()[0];
           }
