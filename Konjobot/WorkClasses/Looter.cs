...
namespace KonjoBot.WorkClasses
{
    ...
    public class Looter
    {
        ...
        public Looter()
        {
            ...
            Core.client.HookProxy.ReceivedCreateOnMapIncomingPacket += HookProxy_ReceivedCreateOnMapIncomingPacket;
            LastCorpeses = new byte[16];

            timer.Execute += Timer_Execute;
        }

        private bool HookProxy_ReceivedCreateOnMapIncomingPacket(Packets.IncomingPacket packet)
        {

            Packets.Incoming.TileAddThing p = (Packets.Incoming.TileAddThing)packet;
            Location location = p.Item.Location.GroundLocation;
            byte stack = p.Item.Location.StackOrder;
            int itemid = p.Item.Id;
            ...
            if (lot.Cap <= Core.client.Player.Capacity)
            {
                ...
                switch (lot.Destination)
                {
                    ...
                    case Constants.LootDestination.LeftHand:
                        i.Move(ItemLocation.FromSlot(Constants.SlotNumber.Weapon));
                        break;
                    case Constants.LootDestination.RightHand:
                        i.Move(ItemLocation.FromSlot(Constants.SlotNumber.Shield));
                        break;
