...
void Packet::AddUint64(UINT64 value)
{
	*(UINT64*)(m_Packet + m_CurrentPos) = value;
	m_CurrentPos += 8;
	m_PacketSize += 8;
...
BYTE* Packet::GetRawPacket(){
    ...
    memcpy(o_Packet, m_Packet, m_PacketSize); //Copy packet to outgoing packet
    return o_Packet;
}
...
UINT64 Packet::ReadUint64(BYTE* buffer, int* offset) {
	DWORD result;
	result =
		buffer[*offset] +
		(buffer[*offset + 1] << 8) +
		...
		(buffer[*offset + 4] << 0x20) +
		(buffer[*offset + 5] << 0x28) +
		(buffer[*offset + 6] << 0x30) +
		(buffer[*offset + 7] << 0x38);
	(*offset) += 8;
	return result;
}
